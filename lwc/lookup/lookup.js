...
export default class Lookup extends NavigationMixin(LightningElement) {
    ...
    set selection(initialSelection) {
        this._curSelection = Array.isArray(initialSelection) ? initialSelection : 
    ...
    hasSelection() {
        //console.log("hasSelection? ", this._curSelection.length > 0);
    ...
    processSelectionUpdate(isUserInteraction) {
        ...
        if (isUserInteraction) {
            const selectedIds = this._curSelection.map((sel) => sel.id);
            this.dispatchEvent(new CustomEvent('selectionchange', { detail: this._curSelection }));
        }
    }
